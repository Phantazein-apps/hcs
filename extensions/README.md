# PCP Extension Namespace Registry
...
## Registered `pcp.*` namespaces (v0.1)

| Namespace | Status | Schema | Sensitivity class |
|---|---|---|---|
| `pcp.health` | Draft (reference example) | [`../schemas/pcp.health.schema.json`](../schemas/pcp.health.schema.json) | **Special category** — separate encryption key required |
| `pcp.residency` | Reserved | — | **Special category** — citizenship(s), visas, residency & tax status |
| `pcp.legal` | Reserved | — | High — matters, contracts of record, attorney contacts |
| `pcp.family` | Reserved | — | High — family structure, dependents, key dates, emergency contacts |
| `pcp.finance` | Reserved | — | High — account references (never credentials), obligations, advisors |
| `pcp.home` | Reserved | — | Medium — addresses, household, vehicles |
...
4. **Third-party namespaces** use reverse-DNS prefixes (`com.example.pets`). The `pcp.*` prefix is assigned only through this registry.
